...
export function TeamStatsPanel({ homeTeam, awayTeam, rows }: TeamStatsPanelProps) {
  return (
    <Card>
      <CardHeader>
        <h3 className="text-sm font-semibold text-slate-100">两队数据对比</h3>
      </CardHeader>
      <CardBody>
        ...
        {rows.length === 0 ? (
          <div className="py-6 text-center text-sm text-slate-500">暂无近期对阵数据</div>
        ) : (
          <div className="divide-y divide-slate-800/70">
            {rows.map((row) => (
              <div key={row.label} className="grid grid-cols-3 items-center py-2 text-sm">
                ...
                <div className="text-center text-slate-400">{row.label}</div>
