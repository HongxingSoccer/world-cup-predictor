  ...
  return (
    <div className="rounded-md surface-card">
      <div className="border-b border-slate-800/70 p-4">
        <h2 className="text-base font-semibold text-slate-200">{t('admin.users.title')}</h2>
        <p className="mt-1 text-xs text-slate-400">{t('admin.users.description')}</p>
      </div>
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-800/70">
          {data.items.map((u) => (
            <tr key={u.id}>
              <td className="px-4 py-2 text-slate-300">{u.id}</td>
              <td className="px-4 py-2 text-slate-300">{u.email ?? '—'}</td>
              <td className="px-4 py-2 text-slate-300">{u.subscription_tier}</td>
              <td className="px-4 py-2 text-slate-400">{new Date(u.created_at).toLocaleString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
