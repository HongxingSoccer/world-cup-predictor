  ...
  return (
    <div className="rounded-md border border-slate-200 bg-white">
      <div className="flex items-center justify-between border-b border-slate-100 p-4">
        <div>
          <h2 className="text-base font-semibold text-slate-800">{t('admin.reports.title')}</h2>
          <p className="mt-1 text-xs text-slate-500">{t('admin.reports.description')}</p>
        </div>
        <div className="flex gap-1 text-xs">
          {[
            { v: '', label: t('admin.reports.filterAll') },
            { v: 'draft', label: t('admin.reports.filterDraft') },
            { v: 'published', label: t('admin.reports.filterPublished') },
          ].map((opt) => (
            <button
              key={opt.v}
              type="button"
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-slate-50 text-left text-xs uppercase text-slate-500">
            ...
            <th className="px-4 py-2">{t('admin.reports.columns.status')}</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {data.items.map((r) => (
            <tr key={r.id}>
              ...
              <td className="px-4 py-2 text-slate-700">{r.match_id}</td>
              <td className="px-4 py-2 text-slate-700">{r.title}</td>
              <td className="px-4 py-2 text-slate-700">{r.model_used}</td>
              <td className="px-4 py-2 text-slate-700">{r.status}</td>
              <td className="px-4 py-2 text-slate-500">{new Date(r.generated_at).toLocaleString()}</td>
