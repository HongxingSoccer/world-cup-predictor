...
export default function AdminPushPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-base font-semibold text-slate-800">{t('admin.push.title')}</h2>
        <p className="mt-1 text-xs text-slate-500">{t('admin.push.description')}</p>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {list.data.items.map((n) => (
              <tr key={n.id}>
                <td className="px-4 py-2 text-slate-700">{n.id}</td>
                <td className="px-4 py-2 text-slate-700">#{n.user_id}</td>
                <td className="px-4 py-2 text-slate-700">{n.channel}</td>
                <td className="px-4 py-2 text-slate-700">{n.notification_type}</td>
                <td className="px-4 py-2 text-slate-700">{n.title}</td>
                <td className="px-4 py-2 text-slate-700">{n.status}</td>
